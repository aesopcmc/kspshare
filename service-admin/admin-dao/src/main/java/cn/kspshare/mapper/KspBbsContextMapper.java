...
import static cn.kspshare.mapper.KspBbsContextDynamicSqlSupport.*;
import static org.mybatis.dynamic.sql.SqlBuilder.*;

import cn.kspshare.domain.KspBbsContext;
...
@Mapper
public interface KspBbsContextMapper {
    ...
    int insert(InsertStatementProvider<KspBbsContext> insertStatement);

    @InsertProvider(type=SqlProviderAdapter.class, method="insertMultiple")
    int insertMultiple(MultiRowInsertStatementProvider<KspBbsContext> multipleInsertStatement);

    @SelectProvider(type=SqlProviderAdapter.class, method="select")
    @ResultMap("KspBbsContextResult")
    Optional<KspBbsContext> selectOne(SelectStatementProvider selectStatement);

    @SelectProvider(type=SqlProviderAdapter.class, method="select")
    @Results(id="KspBbsContextResult", value = {
    ...
    List<KspBbsContext> selectMany(SelectStatementProvider selectStatement);

    @UpdateProvider(type=SqlProviderAdapter.class, method="update")
    int update(UpdateStatementProvider updateStatement);

    default long count(CountDSLCompleter completer) {
        return MyBatis3Utils.countFrom(this::count, kspBbsContext, completer);
    }

    default int delete(DeleteDSLCompleter completer) {
        return MyBatis3Utils.deleteFrom(this::delete, kspBbsContext, completer);
    }
    ...
    default int insert(KspBbsContext record) {
        return MyBatis3Utils.insert(this::insert, record, kspBbsContext, c ->
    ...
    default int insertMultiple(Collection<KspBbsContext> records) {
        return MyBatis3Utils.insertMultiple(this::insertMultiple, records, kspBbsContext, c ->
            c.map(oid).toProperty("oid")
            .map(themeId).toProperty("themeId")
            .map(contextType).toProperty("contextType")
            .map(profile).toProperty("profile")
    ...
    default int insertSelective(KspBbsContext record) {
        return MyBatis3Utils.insert(this::insert, record, kspBbsContext, c ->
    ...
    default Optional<KspBbsContext> selectOne(SelectDSLCompleter completer) {
        return MyBatis3Utils.selectOne(this::selectOne, selectList, kspBbsContext, completer);
    }

    default List<KspBbsContext> select(SelectDSLCompleter completer) {
        return MyBatis3Utils.selectList(this::selectMany, selectList, kspBbsContext, completer);
    }

    default List<KspBbsContext> selectDistinct(SelectDSLCompleter completer) {
        return MyBatis3Utils.selectDistinct(this::selectMany, selectList, kspBbsContext, completer);
    }

    default Optional<KspBbsContext> selectByPrimaryKey(Long oid_) {
        return selectOne(c ->
            c.where(oid, isEqualTo(oid_))
        );
    }
    ...
    default int update(UpdateDSLCompleter completer) {
        return MyBatis3Utils.update(this::update, kspBbsContext, completer);
    }

    static UpdateDSL<UpdateModel> updateAllColumns(KspBbsContext record, UpdateDSL<UpdateModel> dsl) {
    ...
    static UpdateDSL<UpdateModel> updateSelectiveColumns(KspBbsContext record, UpdateDSL<UpdateModel> dsl) {
    ...
    default int updateByPrimaryKey(KspBbsContext record) {
        return update(c ->
    ...
    default int updateByPrimaryKeySelective(KspBbsContext record) {
        return update(c ->
